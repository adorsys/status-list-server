...
Concrete integrations belong in `src/adapters`. The SQL implementation is
`adapters::sea_orm::SeaOrmStatusListRepository`, which delegates to the
configured SeaORM backend (Postgres, MySQL, or SQLite); the cache
implementation is `adapters::cache::MokaStatusListCache`; certificate,
...
The default `server` feature selects the HTTP and configured SQL/Redis/AWS stack;
