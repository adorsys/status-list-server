= A Practical Guide to the OAuth Status List Server (with Rust)
:toc: macro
:toclevels: 2
:icons: font
:imagesdir: ..
:diagram-server-url: https://kroki.io
:diagram-server-type: kroki

toc::[]

[.lead]
If you issue or verify tokens, you need a fast, scalable way to tell whether a token is still valid without calling the issuer every time. A Status List Server, as defined in the https://datatracker.ietf.org/doc/draft-ietf-oauth-status-list/[IETF OAuth Status List draft], solves this by publishing compact, cacheable “status list tokens” that verifiers can fetch and reuse.
...
image::flow.png[Flow from specification, align=center]

_Issuers publish compact status lists once; relying parties cache and reuse them._
...
- Formats
  ...
  * Status list payload compression follows the specification; HTTP gzip compression is applied independently at the transport layer.
- Publisher workflow
...
- Reliability & ops
  ...
  * TTL caches and DB indices for performance
  * Multi-arch static builds and Helm deployment options

=== API quick look

- `GET /` → "Status list Server"
- `GET /health/live` → liveness probe (`/health` remains a compatibility alias)
- `GET /health/ready` → dependency-aware readiness probe
- `POST /api/v1/credentials` → register an issuer’s public key (JWK)
- `PUT /api/v1/status-lists/{list_id}/statuses` (auth) → create a status list
- `PATCH /api/v1/status-lists/{list_id}/statuses` (auth) → update a status list
- `GET /api/v1/status-lists/{list_id}` (public) → gzip-compressed JWT or CWT
