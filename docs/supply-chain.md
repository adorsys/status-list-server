...
`.github/workflows/scheduled-image-scan.yml` runs nightly and on dispatch. It scans the **released** images rather than building anything: for each variant it resolves `latest-<variant>` to an index digest, resolves each architecture's child manifest under it, and runs the same `scripts/derive-trivy-reports.sh` and `scripts/vuln-gate.sh` the release path runs. Three scripts are shared with `deploy.yml` precisely so the two results are the same measurement rather than two similar ones.
...
Two structural details are load-bearing. The per-variant legs render the gate but do not decide the run — a single verdict is aggregated afterwards, so the counts, the tracking issue and the exit code cannot disagree about what was found across five variants. And every step that reports is gated on the aggregation having succeeded: a Trivy database or registry failure fails the run and files nothing, because an issue titled as if a vulnerability were found is how a tracking issue stops being read.
...
**One tracking issue, not one per run.** It is located by an HTML marker in its body rather than by a label, because `gh issue create --label` fails outright when the label does not exist and the notification path must not break on repository configuration. The body carries the previous advisory ID set and is rewritten every run; a **comment** is posted only when that set changes, and names what was added and removed. A findings set that is stable across nights is therefore silent. When findings clear, the issue is updated to say so and **left open** — closing an issue someone is mid-triage on is a worse failure than a stale-looking open one, and reopening loses its place in whatever board or filter it was in.
...
- **The Trivy vulnerability database is fetched fresh on every run**, unauthenticated, from a third party. On the release path that makes a network blip or a GHCR rate limit into a failed release, surfacing as the security gate failing, which reads as "a vulnerability was found". Tracked in [#405](https://github.com/adorsys/status-list-server/issues/405). The nightly is defended against the same confusion rather than fixed: every reporting step is gated on the scan having produced parseable findings, so an unreachable database fails the run and files no issue.
