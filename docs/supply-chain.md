...
Both `deploy.yml` (the `scan-image` job) and `scheduled-image-scan.yml` (the `rescan` job) persist the Trivy vulnerability database across workflow runs using an explicit `actions/cache` step. The cache key is `trivy-db-<os>-<run_id>` with a `restore-keys: trivy-db-<os>-` fallback, so a cache miss restores the most recent DB rather than starting empty.

Before each scan, `scripts/ensure-trivy-db.sh` runs `trivy image --download-db-only` to refresh the DB. If the refresh succeeds, the scan uses a fresh database. If the refresh fails (network blip, rate limit, upstream outage), the script falls back to the cached DB:

- If the cached DB is **within 24 hours** of its last update, the scan proceeds with a `::warning::` annotation and a step-summary entry reporting the DB age (e.g., "Using cached vulnerability database (5 hours old)"). This is the "degrade, don't fail" posture.
- If the cached DB is **older than 24 hours**, the script fails the job with an error. The 24-hour bound matches Trivy's own up-to-date threshold and is the documented staleness limit.
- If **no cached DB exists** (first run ever, or cache was purged), a fetch failure is a hard error — there is nothing to degrade to.

The first run after this change is a cold cache and still downloads ~108 MiB. The resilience benefit (degrade-on-failure, no fresh fetch on cache hit) is realized from the second run onward.

The DB is global (variant- and architecture-independent), so a single cache entry serves all five variant matrix legs and both `amd64`/`arm64` scans. The `trivy-action` scans run with `cache: false`, `skip-setup-trivy: true`, and a `.trivy.yaml` config that sets `db.skip-update: true`, ensuring the scan never re-fetches — freshness is decided entirely by the pre-warm step.
