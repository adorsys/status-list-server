...
`docker compose up` starts PostgreSQL by default and builds the container with `postgres,aws-s3,aws-secrets,aws-route53,acme` features enabled. To run the MySQL service instead:
