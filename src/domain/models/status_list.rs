...
use crate::domain::ports::{StatusListCache, StatusListHistoryRepo, StatusListRepo};
...
impl StatusListRecord {
    ...
    pub async fn publish(
        repo: &dyn StatusListRepo,
        history_repo: Option<&dyn StatusListHistoryRepo>,
        list_id: String,
        issuer: Issuer,
        sub: String,
        ...
        if repo.find(&record.list_id).await?.is_some() {
        ...
        match history_repo {
            Some(_) => {
                ...
                repo.insert_with_snapshot(record.clone(), snapshot).await?;
            }
            None => {
                repo.insert(record.clone()).await?;
            }
        }
    ...
    pub async fn update_statuses(
        ...
        let landed = match history_repo {
            Some(_) => {
                let snapshot = build_snapshot(&existing, token_exp_secs);
                repo.update_with_snapshot(existing.clone(), previous_updated_at, snapshot)
                    .await?
            }
            None => repo.update(existing.clone(), previous_updated_at).await?,
        };
        ...
        invalidate_after_commit(cache, &existing.list_id).await;
        Ok(existing)
    }
    ...
    pub async fn get(
        ...
        if let Some(record) = cache.get(list_id).await? {
            return Ok(record.as_ref().clone());
        }
        let record = repo.find(list_id).await?.ok_or(StatusListError::NotFound)?;
        cache.put(record.as_ref().clone()).await?;
        Ok(record.as_ref().clone())
    }

    /// List all published status list URIs.
    pub async fn list_uris(repo: &dyn StatusListRepo) -> Result<Vec<String>, StatusListError> {
        repo.list_uris().await
    }
}
...
impl StatusListSnapshot {
    ...
    pub async fn cleanup(
        ...
        let Some(history) = history_repo else {
            return Ok(0);
        };
