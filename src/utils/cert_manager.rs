...
impl CertManager {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            acme_http_client_factory: Some(Box::new(move || {
                Box::new(http_client.clone()) as Box<dyn HttpClient>
            })),
    ...
    pub async fn cert_chain_parts(&self) -> Result<Option<Vec<String>>, CertError> {
        ...
        if let Some(cert_data) = self.certificate().await? {
            let certs = X509Pem::iter_from_buffer(cert_data.certificate.as_bytes())
                .map(|cert| {
