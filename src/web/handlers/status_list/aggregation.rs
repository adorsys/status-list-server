...
mod tests {
    ...
    async fn test_aggregation_returns_all_status_list_uris() {
        let status_list1 = StatusListRecord {
            ...
            status_list: StatusList { bits: 1, lst: "foo".to_string() },
            sub: "https://example.com/statuslists/list1".to_string(),
        };
        let status_list2 = StatusListRecord {
            list_id: "list2".to_string(),
            issuer: "issuer2".to_string(),
            status_list: StatusList { bits: 1, lst: "bar".to_string() },
            sub: "https://example.com/statuslists/list2".to_string(),
        };
        let mock_db = MockDatabase::new(DatabaseBackend::Postgres)
            .append_query_results::<status_lists::Model, Vec<_>, _>(vec![vec![
                status_list1.clone(), status_list2.clone()
            ]])
            .into_connection();
        let app_state = test_app_state(Some(Arc::new(mock_db))).await;
        let app = Router::new().route("/aggregation", axum::routing::get(aggregation)).with_state(app_state);
        let response = app
            .oneshot(axum::http::Request::builder().uri("/aggregation").body(axum::body::Body::empty()).unwrap())
            .await
            .unwrap();
        ...
        assert!(result.status_lists.contains(&"https://example.com/statuslists/list1".to_string()));
        assert!(result.status_lists.contains(&"https://example.com/statuslists/list2".to_string()));
    }
} 